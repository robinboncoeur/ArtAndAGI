...
While this all seems like a lot of work, the artist ends up with a setting, a consistent face (via LoRA) and attire (which can *also* be controlled by LoRA. All that lacks is a **story**.
...
Incidently, I still have my 8-gig card. I'm going to have a go, running this workflow using a GGUF instead of a .safetensors file. We'll see what happens.
